...
// 1. Initialize OpenRouter client using your OpenAI API key
...
function extractFirstJson(text: string): string | null {
  ...
  // 1) Prefer fenced blocks (safely written to avoid markdown parser breaks)
  const fence = new RegExp('[`]{3}json\\s*([\\s\\S]*?)\\s*[`]{3}', 'i').exec(text);
  if (fence?.[1]) {
    return fence[1].trim();
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ---- Call model (ask for strict JSON) ----
    const response = await generateText({
      // 2. Uses the custom OpenRouter instance with the openrouter model prefix
    ...
    // ---- Robust parsing & normalization ----
    // 1) Strip json code fences if present safely
    const defenced = raw.replace(/[`]{3}(?:json)?\s*|\s*[`]{3}/gi, "").trim();
    ...
    try {
      ...
      const match = defenced.match(/\{[\s\S]*\}|\[[\s\S]*\]/);
      if (!match) {
        return NextResponse.json(
          { error: "No JSON found in model output." },
          { status: 422 }
